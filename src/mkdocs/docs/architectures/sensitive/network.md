...
The following subnets are defined by the _AWS Secure Environment Accelerator Architecture_:
...
2. The Microsoft AD should be 'shared' with the account the EC2 instance resides in (The _AWS Secure Environment Accelerator Architecture_ recommends these directories are shared to workload accounts);
...
A sandbox VPC, not depicted, may be included in the _AWS Secure Environment Accelerator Architecture_. This is **not** connected to the Transit Gateway, Perimeter VPC, on-premises network, or other common infrastructure. It contains its own Internet Gateway, and is an entirely separate VPC with respect to the rest of the _AWS Secure Environment Accelerator Architecture_.
