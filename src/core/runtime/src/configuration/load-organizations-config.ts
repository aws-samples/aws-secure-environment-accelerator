 ...
 */
 ...
 export const handler = async (input: LoadConfigurationInput): Promise<LoadOrganizationConfigurationOutput> => {
   ...
   const { configFilePath, configRepositoryName, configCommitId, acceleratorPrefix, parametersTableName, baseline } = input;
   ...
   if (baseline === 'CONTROL_TOWER'){
     ...
     const filteredConfigurationAccounts = configurationAccounts.filter(function(currentElement){
       return currentElement.accountKey !== 'management' && currentElement.accountKey !== 'security' && currentElement.accountKey !== 'log-archive'});
