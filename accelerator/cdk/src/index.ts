...
async function main() {
  ...
  let configBranchName = env.CONFIG_BRANCH_NAME || 'master';
  if (configBranchName.startsWith('v')) {
    configBranchName = `bump/${configBranchName}`;
  }
  ...
  console.log(`  Branch: ${configBranchName}`);
