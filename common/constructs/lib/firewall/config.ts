...
export interface FirewallConfigParameters {
  Hostname: string;
  VPCCIDR: string;
  [key: string]: string;
}
...
export interface FirewallConfigProps {
  parameters: FirewallConfigParameters;
}

export class FirewallConfig extends cdk.Construct {
  private readonly config: s3assets.Asset;

  constructor(scope: cdk.Construct, id: string, props: FirewallConfigProps) {
    super(scope, id);
