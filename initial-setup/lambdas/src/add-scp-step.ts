...
export const handler = async (input: AddScpInput) => {
  ...
  for(const scp of scps) {
    ...
    if(pbmmPolicyNames.includes(pbmmPolicyName)) {
      ...
      const updatePolicyResponse = await organizations.updatePolicy(JSON.stringify(JSON.parse(scpContent)),
      scp.description,
      pbmmPolicyName,
      existingPolicyId!);
      ...
      // as policy max size should be less than 5,120 bytes
      const createPolicyResponse = await organizations.createPolicy(JSON.stringify(JSON.parse(scpContent)),
      scp.description,
      pbmmPolicyName,
