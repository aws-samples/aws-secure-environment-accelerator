...
import { PeeringConnectionConfig, VpcConfig, AccountConfig, OrganizationalUnitConfig, VpcConfigType, AcceleratorConfig } from '@aws-pbmm/common-lambda/lib/config';
...
async function main() {
  ...
  const createIamRole = (roleName: string, sourceAccount:string, targetAccount: string): string => {
    ...
    const peeringRole = new iam.Role(iamRolePeering, roleName, {
      ...
      assumedBy: new iam.CompositePrincipal(
        new iam.ArnPrincipal(`arn:aws:iam::${getAccountId(accounts, targetAccount)}:role/${context.acceleratorExecutionRoleName}`) // AccountPrincipal(getAccountId(accounts, accountKey))
      )
    });
  ...
  for( const [account, accountConfig] of Object.entries(accountConfigs)){
    ...
    const pcx = new PeeringConnectionDeployment(pcxDeployment, `PcxDeployment${pcxConfig.source}${pcxConfig["source-vpc"]}`, {
      vpcConfig,
      peerVpcConfig,
      accounts,
      accountKey,
...
export function getVpcConfig (accountConfigs: VpcConfigs, accountKey: string, vpcName: string) : VpcConfig | undefined{
  const vpcConfig = Object.entries(accountConfigs).find(x =>
    (
      x[0] === accountKey
      && x[1].vpc
      && x[1].vpc.name === vpcName
    ) || (
     x[1].vpc
     && x[1].vpc.deploy === accountKey
     && x[1].vpc.name === vpcName
    ))?.[1].vpc;
  return vpcConfig;
}
