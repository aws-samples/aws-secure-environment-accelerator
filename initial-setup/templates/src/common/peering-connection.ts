...
import { VpcConfig, PeeringConnectionConfig, AccountConfig, OrganizationalUnitConfig, AcceleratorConfig } from '@aws-pbmm/common-lambda/lib/config';
...
export namespace PeeringConnection{
    ...
    export function getVpcConfigForPcx(accountKey: string, config: AccountConfig | OrganizationalUnitConfig): VpcConfigsOutput | undefined{
        ...
        if (!vpcConfig.deploy) {
            console.warn(`Skipping Peering Connection creation for Account/Organization unit "${accountKey}" as 'deploy' is not set`);
            return;
        }
