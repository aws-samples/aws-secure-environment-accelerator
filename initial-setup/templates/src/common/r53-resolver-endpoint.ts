  ...
  constructor(parent: cdk.Construct, name: string, props: Route53ResolverEndpointProps) {
    ...
    const vpcId = getStackOutput(props.outputs, accountName!, `Vpc${vpcConfig.name}`)
      ...
      for (const [key, subnet] of subnetDefinitions.entries()) {
        ...
        ipAddress.push({
          subnetId: getStackOutput(props.outputs, accountName!, `${vpcConfig.name}Subnet${endpointSubnet?.name}az${key + 1}`),
        });
      }
      ...
      if(inBoundEndpoint){
        const lambdaFnc = lambda.Function.fromFunctionArn(this, 'CfnInBoundEndpointIpPooler', props.context.cfnDnsEndopintIpsLambdaArn);
      ...
      if(outBoundEndpoint){
        const lambdaFnc = lambda.Function.fromFunctionArn(this, 'CfnOuBoundEndpointIpPooler', props.context.cfnDnsEndopintIpsLambdaArn);
        // Create CfnCustom Resource to get IPs which are alloted to InBound Endpoint
