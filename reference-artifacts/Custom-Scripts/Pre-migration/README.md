...
- You can run the scripts from your local workstation. If you are filtering egress trafic from your corporate network you need to ensure [outbound connectivity to AWS service endpoints](#network-timeout-or-connectivity-issue-running-the-upgrade-tool). 
...
> **NOTE**: By default the upgrade tool uses `ca-central-1` as the home region. If you use a different home region you need to set the AWS_REGION environment varianle before running `migration-config`. e.g. `AWS_REGION=eu-west-1 yarn run migration-config`
...
See information about the [Central Logging bucket CMK](docs/KMS.md#central-logging-bucket) for more details and how to fix the issue.
