...
export class Compare {
  ...
  enabledRegions: string[];
    sts: STS;
    constructor(config: Config, compareConfig: { accountList: Account[], enabledRegions: string[], acceleratorConfig: AcceleratorConfig, }) {
    ...
    async GetSubscriptionFilterDeliveryArns(destinationArnMatch: string, region: string) {
      const environments = getEnvironments(this.accountList, [region]);
      const logsClientMap = await this.getLogsClientMap(environments);
      const logGroupsPromises = environments.map(environment => this.getLogGroups(logsClientMap, environment.accountId, environment.region));
      const logGroups = await Promise.all(logGroupsPromises);
      const driftPromises = logGroups.map(logGroup => this.getSubscriptionFiltersDrifts(logGroup.account, logGroup.region, logGroup.cwl!, logGroup.logGroups, destinationArnMatch));
      const subscriptionFilterDrifts = await Promise.all(driftPromises);
      await writeFile('./outputs/subscription-filter-drifts.json', JSON.stringify(subscriptionFilterDrifts.flat(), null, 2));
    }
  ...
  async getSubscriptionFiltersDrifts(account: string, region: string, cwl: CloudWatchLogs, logGroups: (string | undefined)[], destinationArnMatch: string) {
    const validLogGroups = logGroups.filter(logGroup => logGroup !== undefined);
    const subscriptionFilterDriftPromises = validLogGroups.map(validLogGroup => this.getSubscriptionFilterDrift(account, region, validLogGroup!, cwl, destinationArnMatch));
  ...
  async getSubscriptionFilterDrift(account: string, region: string, logGroupName: string, cwlClient: CloudWatchLogs, destinationArnMatch: string) {
    const describeLogGroupFilterResponse = await throttlingBackOff(() => cwlClient.describeSubscriptionFilters({ logGroupName }).promise());
    ...
    const subscriptionFiltersDrift = subscriptionFilters.filter(subscriptionFilter => !subscriptionFilter.destinationArn?.includes(destinationArnMatch));
    if (
      subscriptionFiltersDrift.length > 0
    ) {
      return {
        account: account,
        region: region,
        logGroupName: logGroupName,
        subscriptionFilterDrift: subscriptionFiltersDrift.map(subscriptionFilter => subscriptionFilter.destinationArn)
      }
    }
    return undefined;
  }
    ...
    async getAccountListFromDDB(parametersTableName: string, region: string) {
      ...
      async getLogsClientMap(environments: Environment[]) {
        ...
        for (const environment of environments) {
          logsClientPromises.push(
            this.createLogsClients(this.assumeRoleName, environment.accountId, environment.region),
          );
        }
