...
export type IamUserConfig = t.TypeOf<typeof IamUserConfigType>;
...
export type IamPolicyConfig = t.TypeOf<typeof IamPolicyConfigType>;
...
export type IamRoleConfig = t.TypeOf<typeof IamRoleConfigType>;
...
export const DeploymentConfigType = t.interface({
  ...
  'firewalls': t.optional(
    t.array(t.union([FirewallEC2ConfigType, FirewallCGWConfigType, FirewallAutoScaleConfigType])),
  ),
...
export const GlobalOptionsConfigType = t.interface({
  ...
  'supported-regions': t.array(t.region),
  'keep-default-vpc-regions': t.defaulted(t.array(t.nonEmptyString), []),
...
export class AcceleratorConfig {
  ...
  getCustomerManagedPoliciesByAccount(accountKey: string) {
    ...
    if (accountConfig.iam?.policies) {
      policies.push(...accountConfig.iam.policies.flatMap((policy) => policy['policy-name']));
    }
  ...
  getVpcConfigs(): ResolvedVpcConfig[] {
    ...
    for (const [ouKey, ouConfig] of Object.entries(this['organizational-units'])) {
