...
- [13. Is it possible to deploy ASEA on top of an AWS Organization that I have already installed the AWS Landing Zone (ALZ) into?](#13-is-it-possible-to-deploy-asea-on-top-of-an-aws-organization-that-i-have-already-installed-the-aws-landing-zone-alz-ALZ-into)
...
### 12. I deployed AWS Managed Active Directory (MAD) as part of my depoloyment, how do I manage Active Directory domain users, groups, and domain policies after deployment?
...
While the initial deployment allows you to provide your own certificates (either self-signed or generated by a CA), we would recommend leveraging AWS Certificate Manager (ACM) to easily provision, manage, and deploy public and private SSL/TLS certificates for use with your ALBs. ACM helps manage the challenges of maintaining certificates, including certificate renewals so you don’t have to worry about expiring certificates. To update the ALB cert, create a new certificate resource and update the ALB’s “cert-name” to use the new certificate in the config.json.

### 16. What level of Support will the ASEA have from AWS Support?

The majority of the solution includes native AWS services, and are covered by AWS support which can be accessed by opening a Support Case.  As code is involved, anything specifically related to the code would be "best effort".  For the solution/code, the first line of support is typically your local AWS team (your SA, TAM, Proserve and/or AWS Partner).  Issues can be filed against the GitHub repository, with most issues associated to installation.

### 17. Why do we have rsyslog servers? I thought everything was sent to CloudWatch?

The rsyslog servers will accept logs for appliances that do not support natively CloudWatch. For example, the Fortigate firewalls will send their logs to rsyslog which will in turn store them to CloudWatch.

### 18. Can you deploy the solution without Fortinet Firewall Licenses?

Yes, the firewalls will come up and route traffic, but you have no mechanism to manage the firewalls/change the configuration. If this is a test deployment, please work directly with your local Fortinet account team to discuss any options for temporary evaluation licenses.

You can do an install and deploy the Fortinet's without eval keys, the firewalls will come up and route traffic, but you have NO mechanism to manage the firewalls/change the config until the customer provides the eval keys.
...
In essence you can define a VPC in one of three major sections:
a) within an account in mandatory-account-configs -> the account 
b) within an organization unit organizational-units -> the OU
